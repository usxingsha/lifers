...
# Auto-detect RAM and set safe tier cap if not already set
if [[ -z "${LIFERS_ESCALATE_MAX_TIER:-}" ]]; then
  if command -v free >/dev/null 2>&1; then
    _RAM_MB=$(free -m 2>/dev/null | awk '/Mem:/{print $2}' || echo "0")
  else
    _RAM_MB=0
  fi
  if [[ "$_RAM_MB" -gt 0 ]]; then
    if [[ "$_RAM_MB" -lt 4096 ]]; then
      export LIFERS_ESCALATE_MAX_TIER=8
      echo "[lifers-loop] low RAM detected (${_RAM_MB}MB) — auto capping LIFERS_ESCALATE_MAX_TIER=8"
    elif [[ "$_RAM_MB" -lt 8192 ]]; then
      export LIFERS_ESCALATE_MAX_TIER=14
      echo "[lifers-loop] mid RAM detected (${_RAM_MB}MB) — auto capping LIFERS_ESCALATE_MAX_TIER=14"
    elif [[ "$_RAM_MB" -lt 16384 ]]; then
      export LIFERS_ESCALATE_MAX_TIER=18
      echo "[lifers-loop] 8-16GB RAM detected (${_RAM_MB}MB) — auto capping LIFERS_ESCALATE_MAX_TIER=18"
    fi
  fi
fi
...
_OOM_MARKER="$ROOT/weights/.last_oom_tier"
...
while true; do
  ...
  case "$mode" in
    ...
    run)
      ...
      # Check for OOM marker written by escalate script
      if [[ -f "$_OOM_MARKER" ]]; then
        oom_tier=$(cat "$_OOM_MARKER" 2>/dev/null || echo "")
        if [[ -n "$oom_tier" ]] && [[ "$oom_tier" -gt 0 ]]; then
          echo "[lifers-loop] OOM at tier $oom_tier — auto-capping LIFERS_ESCALATE_MAX_TIER=$((oom_tier - 1))"
          echo "[lifers-loop] To retry: add RAM/swap, then: rm $_OOM_MARKER && echo run > $CTL"
          export LIFERS_ESCALATE_MAX_TIER="$((oom_tier - 1))"
          # Don't rm the marker — user must explicitly delete it to re-enable that tier
        fi
      fi
