...
from lifers.agent import _META_CAP_SHORT, _META_SELF_RE
from lifers.daily_intents import (
    cn_web_query_line,
...
# Module-level cache to avoid re-creating Planner() per call
# when no explicit planner is passed.  Safe because plan() is
# deterministic (text → ToolCall[]).
_PLANNER_CACHE: Planner | None = None
...
def _get_planner(explicit: Planner | None = None) -> Planner:
    if explicit is not None:
        return explicit
    global _PLANNER_CACHE
    if _PLANNER_CACHE is None:
        _PLANNER_CACHE = Planner()
    return _PLANNER_CACHE


def infer_dialogue_route(
    user_text: str,
    has_context_prefix: bool,
    *,
    emit: bool = True,
    ...
    :param planner: 可选外部 Planner 实例（避免反复构造 + 调用两次 plan()）。
    """
