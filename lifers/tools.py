...
class WebFetchTool(Tool):
    ...
    def execute(self, call: ToolCall) -> ToolResult:
        ...
        req = urllib.request.Request(url, headers={"User-Agent": "lifers/0.1"})
        try:
            with _http_open(req, timeout=call.timeout_ms / 1000) as resp:
...
class FsWritePatchTool(Tool):
    ...
    def execute(self, call: ToolCall) -> ToolResult:
        ...
        from lifers.safe_file_backup import safe_replace_file_text
    ...
    def verify(self, call: ToolCall, prior: Optional[ToolResult] = None) -> ToolResult:
        ...
        from lifers.safe_file_backup import commit_journal
    ...
    def rollback(self, call: ToolCall, prior: Optional[ToolResult] = None) -> ToolResult:
        from lifers.safe_file_backup import restore_from_hint, rollback_hint_from_prior
...
class KbSearchTool(Tool):
    ...
    def execute(self, call: ToolCall) -> ToolResult:
        from lifers.memory import fts5_search
...
def build_default_registry() -> ToolRegistry:
    ...
    try:
        from lifers.plugin_loader import register_plugins
