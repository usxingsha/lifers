...
用法（在 lifers_brain 根）:
  ...
  LIFERS_BRAIN_ROOT=/path/to/lifers_brain python scripts/check_lifers_llm_ready.py
"""
...
def main() -> int:
    brain = Path(os.environ.get("LIFERS_BRAIN_ROOT", str(Path(__file__).resolve().parent.parent))).resolve()
    ...
    if not out["ok"]:
        out["hints"].append("缺少 weights/lifers_markov.json 与 lifers_transformer.json：在 lifers_brain 执行 python scripts/run_pipeline.py 或按 playbook 训练/同步权重。")
    if not op.is_dir():
