...
得到一块单独给 lifers_brain 用的桌面入口，行为上接近「专用小发行版」。

启动
----
Windows（在 lifers_brain 仓库里）:
  powershell -ExecutionPolicy Bypass -File tools\lifers_editor\lifers-editor.ps1

Linux / Kali（在 lifers_brain 根目录）:
...
--------------------
通常有两部分:
  1) **lifers_brain** 代码与权重: 由 tar/scp 解压到某路径（如 /home/kali/lifers/lifers_brain）
