...
- **主仓**：Lifers 便携根（目录名 **`lifers/`**，与 GitHub 一致）；本目录为 **vendor 源码树**，以 Kali 上 `~/lifers/third_party/claw-code/rust` 为权威对照做合并。
...
| 项 | 值 |
|----|-----|
| Kali 源路径 | `/home/kali/lifers/third_party/claw-code/rust` |
| Lifers 相对路径 | `third_party/claw_code_rust/`（便携根目录名 **`lifers`**） |
| 传输包 | `lifers/dist/claw_rust_merged.tgz`（由 Kali 端 `tar`，排除见下） |
...
`crates/api` 等目录包含 **上游 HTTP/兼容客户端实现**，属审计与对照用途；**不代表** Lifers 已接入或运行这些 API。产品边界以 `lifers/config/stack.json` 与 `openclaw_manifest.json` 为准。

## 与 Kali 训练包的关系

`lifers/scripts/package_rs_for_kali.ps1` 仅打包 **`lifers/`** 子树；`third_party/claw_code_rust` 在便携根 **`lifers/third_party/`**，**不会**自动进入 `lifers_kali.tar.gz`。若需在 Kali 上携带同一份 vendor，请另行同步整个便携根或使用 `scp`/`rsync` 本目录。

## 清单文件

- 机器可读：`lifers/config/claw_code_rust_vendor.json`
- OpenClaw 并列对照：`lifers/config/openclaw_manifest.json`（能力域中有 `claw_code_rust` 分项）

## 再同步步骤（维护者）

1. Kali：`cd ~/lifers/third_party/claw-code/rust && tar czf /tmp/claw_rust_merged.tgz --exclude=./target --exclude='./.claw/sessions' --exclude='./.claude/sessions' .`
2. Windows：`scp kali:/tmp/claw_rust_merged.tgz lifers/lifers/dist/`（便携根目录名为 **`lifers`**）
