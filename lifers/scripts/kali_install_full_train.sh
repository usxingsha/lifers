...
#   LIFERS_ROOT     若已固定 brain 绝对路径，可跳过自动探测
...
resolve_brain() {
  if [[ -n "${LIFERS_ROOT:-}" ]] && [[ -f "$LIFERS_ROOT/scripts/kali_train_weights.sh" ]]; then
    echo "$LIFERS_ROOT"
    return
  fi
  # 从本脚本所在仓库运行（已解压）
  ...
  echo "[kali-install] 无法定位 brain 根。请先解压 tar 到 $DEST，或设置 LIFERS_ROOT，或在已解压目录下执行本脚本。" >&2
  exit 1
}
