...
<#
.SYNOPSIS
  运行每日 AI 健康评测（逻辑自洽/全链检查/桥接延迟/上下文吞吐），写入 lifers/state/daily_health/。
...
.NOTES
  计划任务（每天一次，例如 04:30）：
  schtasks /Create /TN "LifersDailyAIHealth" /TR "powershell -NoProfile -ExecutionPolicy Bypass -File C:\path\to\lifers\lifers\scripts\run_daily_ai_health.ps1" /SC DAILY /ST 04:30 /RL LIMITED
#>
param(
