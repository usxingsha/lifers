...
claw-code/rust 源码并入（vendor）：目录 third_party/claw_code_rust 以 Kali 上 ~/lifers/third_party/claw-code/rust 为权威对照（排除 target 与会话缓存后打包同步）；见 lifers_brain/config/claw_code_rust_vendor.json 与 third_party/claw_code_rust/LIFERS_MERGE.md。不在 Lifers 内启用其 HTTP/API 运行时。
