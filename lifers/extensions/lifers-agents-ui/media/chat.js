(function () {
  ...
  function applyLifersConfig(cfg) {
    ...
    const verEl = document.getElementById('lifers-ext-ver');
    if (verEl && cfg.extensionVersion) {
      verEl.textContent = 'v' + cfg.extensionVersion;
      const b = cfg.bridgeTimeoutSec != null ? String(cfg.bridgeTimeoutSec) : '';
      const e = cfg.edgeGenerateTimeoutSec != null ? String(cfg.edgeGenerateTimeoutSec) : '';
      verEl.title =
        'lifers-agents-ui v' +
        cfg.extensionVersion +
        (b ? ' · Bridge 外层约 ' + b + 's' : '') +
        (e !== '' ? ' · 内层 generate ' + e + 's' : '');
    }
  }
