...
# Env set by train_lifers_escalate: LIFERS_BRAIN_ROOT LIFERS_CHECKPOINT_JSON LIFERS_CHECKPOINT_B LIFERS_CUMULATIVE_EST
set -euo pipefail
ROOT="${LIFERS_BRAIN_ROOT:-}"
JSON="${LIFERS_CHECKPOINT_JSON:-}"
B="${LIFERS_CHECKPOINT_B:-?}"
if [[ -z "$ROOT" || -z "$JSON" ]]; then
  echo "[post-cp] missing LIFERS_BRAIN_ROOT or LIFERS_CHECKPOINT_JSON" >&2
  exit 0
fi
