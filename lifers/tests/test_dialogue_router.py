...
class DialogueRouterTests(unittest.TestCase):
    ...
    def test_poem_request_routes_full_pipeline(self) -> None:
        r = infer_dialogue_route("写首诗", False, emit=False)
        self.assertEqual(r.kind, TaskKind.FULL_PIPELINE)
        self.assertEqual(r.reason, "rewrite_remember_or_longform")

    def test_couplet_request_routes_full_pipeline(self) -> None:
        r = infer_dialogue_route("写一副对联", False, emit=False)
        self.assertEqual(r.kind, TaskKind.FULL_PIPELINE)
        self.assertEqual(r.reason, "rewrite_remember_or_longform")
