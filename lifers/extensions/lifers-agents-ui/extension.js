...
function activate(context) {
  ...
  context.subscriptions.push(
    vscode.workspace.onDidChangeWorkspaceFolders(() => {
      ...
      setTimeout(() => {
        const r2 = getBrainRoot();
        if (r2) controller.bootstrapDefaultSessionIfNeeded(r2);
        controller.refreshChatStateIfPanelOpen(getBrainRoot());
      }, 400);
    })
  );
  ...
  context.subscriptions.push(
    vscode.workspace.onDidChangeConfiguration((e) => {
      ...
      if (e.affectsConfiguration('lifers.brainPathOverride')) {
        controller.resetSessionBootstrapFlag();
        const r = getBrainRoot();
        if (r) controller.bootstrapDefaultSessionIfNeeded(r);
        controller.refreshChatStateIfPanelOpen(r);
        sessionTreeProvider.refresh();
      }
    })
  );
...
/** 无工作区打开时，用于展开 lifers.brainPathOverride 里的 ${workspaceFolder}。 */
function noWorkspaceExpandBase() {
  try {
    const c = process.cwd();
    if (c) return c;
  } catch {
...
/**
 * 未打开文件夹时：从扩展安装目录向上查找含 agent_bridge_once.py 的目录（开发态多在 lifers/extensions/...）。
 * @param {string} startDir extension.js 所在目录
 */
function discoverBrainRootNearExtension(startDir) {
  let cur = path.resolve(startDir);
  for (let i = 0; i < 14; i++) {
    if (hasAgentBridge(cur)) return cur;
    const nested = path.join(cur, 'lifers');
...
function resolveBrainFolder(wsRoot) {
  ...
  const nestedDirs = ['lifers', 'lifers'];
  for (const seg of nestedDirs) {
    const nested = path.join(abs, seg);
    if (hasAgentBridge(nested)) return nested;
  }
  // 仅打开 rs0 等子目录时：在父目录旁查找 lifers（与 lifers.code-workspace 多根布局一致）
...
function getBrainRoot() {
  ...
  const expandBase =
    folders && folders.length ? folders[0].uri.fsPath : noWorkspaceExpandBase();

  /** @param {string} dir */
  const accept = (dir) => {
    const r = path.normalize(path.resolve(String(dir || '')));
    return hasAgentBridge(r) ? r : null;
  };

  if (override) {
    const o = expandWorkspaceFolderVars(override, expandBase);
    const hit = accept(o);
    if (hit) return hit;
  }

  if (!folders || !folders.length) {
    const fromExt = discoverBrainRootNearExtension(__dirname);
  ...
  const prefer = (nm) => {
    ...
    return n === 'lifers' || n === 'lifers' || n === 'lifers brain' || n === 'rs';
  };
  ...
  for (const f of ordered) {
    const hit = accept(resolveBrainFolder(f.uri.fsPath));
    if (hit) return hit;
  }
  ...
  if (!t) {
    ...
    throw new Error(
      se
        ...
        : 'Bridge 无标准输出（请检查 Python 路径与 lifers）'
    );
  }
  try {
    return JSON.parse(t);
...
class LifersAgentsController {
  ...
  constructor(extensionUri, context) {
    ...
    /** @type {string} */
    this._extensionVersion = this._readExtensionVersionFromDisk();
  }

  /** @returns {string} */
  _readExtensionVersionFromDisk() {
    try {
      const pj = JSON.parse(fs.readFileSync(path.join(this.extensionUri.fsPath, 'package.json'), 'utf8'));
      return pj && pj.version ? String(pj.version) : '?';
    } catch {
      return '?';
    }
  }
   ...
   * @returns {number}
   */
  _readBridgeTimeoutMs(conf) {
    const raw = conf.get('bridgeTimeoutMs');
    return Number.isFinite(Number(raw)) ? Math.max(15000, Number(raw)) : 120000;
  }
   ...
   * @returns {number}
   */
  _readEdgeGenerateTimeoutSec(conf) {
    const raw = conf.get('edgeGenerateTimeoutSec');
    let sec = 110;
    if (raw !== null && raw !== undefined && raw !== '') {
      const n = Number(raw);
      if (Number.isFinite(n)) sec = Math.max(0, Math.floor(n));
    }
    return sec;
  }
  ...
  async _remoteTrainStatusJson() {
    ...
    const bp = String(conf.get('kaliBrainPath') || '/home/kali/lifers/lifers').trim();
  ...
  async handleMessage(msg, source) {
    ...
    if (msg.type === 'ready') {
      if (!root) {
        ...
        vscode.window.showWarningMessage(
          '未检测到工作区文件夹：请打开 lifers.code-workspace，或在「用户设置」中填写 lifers.brainPathOverride（lifers 绝对路径）。扩展会尝试从安装目录向上自动探测。 | No folder: open lifers.code-workspace or set lifers.brainPathOverride.'
        );
      } else {
        this._postState(root);
        this.bootstrapDefaultSessionIfNeeded(root);
    ...
    if (!root) {
      vscode.window.showWarningMessage(
        '未检测到工作区文件夹：请打开 lifers.code-workspace，或在「用户设置」中填写 lifers.brainPathOverride。 | No folder: open workspace or set lifers.brainPathOverride.'
      );
      return;
    }
    ...
    if (msg.type === 'send') {
      ...
      try {
        ...
        vscode.window.showErrorMessage('Agents Chat: ' + detail.slice(0, 600));
        this._postReply({ type: 'reply', ok: false, error: detail });
      }
      return;
    }
  ...
  _lifersConfigPayload() {
    const conf = vscode.workspace.getConfiguration('lifers');
    const bridgeMs = this._readBridgeTimeoutMs(conf);
    const edgeSec = this._readEdgeGenerateTimeoutSec(conf);
    return {
      model: 'lifers',
      sandbox: conf.get('sandbox') !== false,
      extensionVersion: this._extensionVersion,
      bridgeTimeoutSec: Math.round(bridgeMs / 1000),
      edgeGenerateTimeoutSec: edgeSec,
    };
  }

  _postConfigToChat() {
    const cfg = this._lifersConfigPayload();
    this._chatPanel?.webview.postMessage({ type: 'lifersConfig', ...cfg });
  }
  ...
  /**
   * 工作区 / brainPathOverride 变化后：若 Chat 已打开，补发 bootstrap，避免仍停留在「无根」时的空会话。
   * @param {string | null | undefined} root
   */
  refreshChatStateIfPanelOpen(root) {
  ...
  bootstrapDefaultSessionIfNeeded(root) {
    ...
    if (!d.sessions || !d.sessions.length) {
      vscode.window.showErrorMessage(
        'Lifers: 未能创建或保存会话（磁盘权限或路径异常）。请重载窗口；仍失败则用「文件 → 打开文件夹」打开 lifers 便携根或 lifers。'
      );
    }
  }
  ...
  async addContextFolder() {
    ...
    if (!isPathInside(root, dir) || !fs.existsSync(dir)) {
      vscode.window.showErrorMessage(
        '目录必须在当前工作区（lifers / rs）内且存在。 | Folder must exist under the open workspace.'
      );
      return;
    }
    try {
  ...
  <div>
    ...
    底部同一栏另有「会话建立」树。物化闭环在仓库：<code>scripts/embodied_tick_once.py</code> + <code>stack.embodied_world</code>（<code>dynamic_npc</code> 仅占位；多体待扩展；先 pause 再同步；tick 会尊重 <code>weights/.train_control</code> 的 pause/stop）。对话路由/推理链说明见 <code>lifers/taskflow/FLOW.md</code>；本机烟测 <code>python scripts/verify_edge_agent_pipeline.py</code>。</p>
    <p class="hint">边缘上 <code>lifers.model=lifers</code> 会加载大体积 <code>lifers_transformer.json</code>（首包可能数十秒级）；日常秒回可改 <strong>lifers.model = markov</strong>。亦可填 <code>lifers.edgeQuickSessionChars</code> / <code>edgeQuickPackChars</code> / <code>edgeQuickStackChars</code> 注入 <code>LIFERS_QUICK_*</code>；<code>lifers.edgeGenerateTimeoutSec</code> 映射 <code>LIFERS_QUICK_GENERATE_TIMEOUT_SEC</code>（内层 generate 墙钟，默认 110s）；<code>lifers.bridgeTimeoutMs</code> 为整次 Python 外层（默认 120s）。CHAT_QUICK 默认在回复末追加 <strong>【本轮·生成锚】</strong>（<code>LIFERS_AGENTS_UI_BRIDGE</code>；<code>LIFERS_QUICK_TIME_FOOTER=0</code> 可关）。</p>
  ...
  async _runWindowsPackAndScp() {
    const root = getBrainRoot();
    if (!root) return '未打开 lifers 工作区，无法打包。';
    const conf = vscode.workspace.getConfiguration('lifers');
...
${bodyInner}
  ...
  <div class="chat-root">
    <header class="chat-toolbar">
      <div class="chat-toolbar-left">
        ...
        <span id="lifers-ext-ver" class="chat-toolbar-ver" title="扩展版本与超时策略">v…</span>
      </div>
  ...
  async _handleSend(root, text, sessionId, contextFiles) {
    ...
    const timeoutMs = this._readBridgeTimeoutMs(conf);
    ...
    const edgeGenSec = this._readEdgeGenerateTimeoutSec(conf);
    ...
    if (!fs.existsSync(script)) {
      const msg =
        '缺少 scripts/agent_bridge_once.py。请打开 lifers/lifers.code-workspace 或 lifers 文件夹；仅打开 rs0 时设 lifers.brainPathOverride（如 /home/kali/lifers/lifers）后 Reload Window。';
      ...
      const { out, errText } = await new Promise((resolve, reject) => {
        const proc = cp.spawn(py, ['-u', script], {
          ...
          env: {
            ...
            LIFERS_QUICK_GENERATE_TIMEOUT_SEC: String(edgeGenSec),
        ...
        const timer = setTimeout(() => {
          ...
          reject(
            new Error(
              `Bridge 外层超时（已等待 ${timeoutMs / 1000}s，整次 Python 无输出）。请调高 **lifers.bridgeTimeoutMs**（毫秒），并保证大于内层 **lifers.edgeGenerateTimeoutSec**（秒→LIFERS_QUICK_GENERATE_TIMEOUT_SEC，默认 110）。首包极慢可暂改 **lifers.model=markov** 或终端跑脚本。`
            )
          );
        }, timeoutMs);
