...
1.注意数据和代码中的乱码和空白问题。
2.当在一个地方停留超过10分钟时就检测是否卡死或者陷入重复的循环中，若是任务循环修复不了就重新修改任务。。
3.同步更新到kali时候检查两个版本的文件和文件内容是否全部同步了 ssh kali@192.168.234.152 root密码是kali。github账号上传仓库时候就检查仓库里的文件目录、文件和数据内容，多余的无用的就删除。
4.不要使用硬编码，自动匹配自动使用。
5.更改/更新和大更新时候，旧数据、文件、训练数据、训练权重等有用正常的就合并，无用的就删除。
6.把大任务拆分成多个goal执行，避免ks和陷入重复循环中。
7.所有数据内容和数据注意质量问题，我要高质量的。