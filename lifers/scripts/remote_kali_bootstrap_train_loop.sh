...
# 假定已通过 tar 或 rsync 更新本仓库到本机（如 ~/lifers/lifers）；从 brain 根或 scripts 下执行均可。
#
# 用法：
#   cd ~/lifers/lifers && bash scripts/remote_kali_bootstrap_train_loop.sh
#   LIFERS_ROOT=/path/to/lifers bash scripts/remote_kali_bootstrap_train_loop.sh
#
# 环境变量（可选）：
...
BR="${LIFERS_ROOT:-$(cd "$SCRIPT_DIR/.." && pwd)}"
BR="$(cd "$BR" && pwd)"
cd "$BR"
