...
**本地模型**：推理只认 **`lifers_brain/weights/lifers_transformer.json`**（及 stack 配置路径）；已删除仓库内玩具后备 **`tiny_transformer_v001.json`**。Kali/本机 **`train_lifers_escalate.py`** 持续写入同一文件时，对话侧按文件更新时间自动加载新权重（边训边用）。**Cursor**：仓库含 **`.cursor/rules/*.mdc`**（免费、可版本化的项目规则）；不捆绑 Cursor 商业安装包。
...
- 持久 HTTP 桥（与 stdin `agent_bridge_once` 同款 JSON）：`lifers_brain/scripts/lifers_gate.py`（默认 `127.0.0.1:55555`，勿对公网暴露）
- 任务流与审计对照：`lifers_brain/taskflow/FLOW.md`（含 **外部审计陈述对照** 表）
...
| OpenClaw 清单 | `lifers_brain/config/openclaw_manifest.json` |
| Rust vendor 清单 | `lifers_brain/config/claw_code_rust_vendor.json` |
| 总 vendor 策略 | `lifers_brain/config/lifers_vendor_map.json` |
| 运维长文 | `lifers_brain/config/lifers_ai_playbook_zh.md` |

## rs0 是什么

多根工作区 **`lifers.code-workspace`** 的首文件夹 **`./rs0`**：可选本地笔记/小实验目录，**允许为空**；运行时与脚本以 **`lifers_brain`** 为准。

## VS Code / VSCodium 扩展

根目录 **`.vscode/extensions.json`** 含 **`lifers.lifers-agents-ui`**。若市场或 Open VSX 未上架该 ID，请用 **`tools/install_agents_extension.ps1`**，或在编辑器中选择 **从文件夹安装扩展** → `lifers_brain/extensions/lifers-agents-ui`。
