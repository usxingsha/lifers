...
机读索引：**`config/lifers_ai_pipeline.json`**（含各阶段源码路径、配置锚点、stderr 标签、聚合自检命令，以及 **`omni_stack_zh`**「全能型」栈位说明）。以下为与之一一对应的总表，便于人工审计「无遗漏」。人读边界见 **`lifers_ai_playbook_zh.md` §13**。
...
| 「无工具 / 无函数调用」 | **`lifers/tools.py`** `build_default_registry()`；`TOOL_PLAN` / `step` 下调度工具。 |
...
| 「Gate 未上传」 | HTTP 入口为 **`lifers/scripts/lifers_gate.py`**（默认 `127.0.0.1:55555`），契约与 `agent_bridge_once` 同源 JSON。 |
...
| `LIFERS_QUICK_GENERATE_TIMEOUT_SEC` | **Agents Bridge 未显式设置时默认约 120s**；`0` 关闭 | CHAT_QUICK 本地 transformer **内层**墙钟；Windows 用线程 `result(timeout=…)`。扩展 **`lifers.edgeGenerateTimeoutSec`**（默认 **110**）会注入本变量，宜 **略小于** **`lifers.bridgeTimeoutMs`**（默认 **120000ms**）。与外层 Bridge **独立**。 |
...
**编排（不产生新 tool name）**：`smart`/`智搜` 在 `agent._step_core` 内手写 `kb_search`→条件 `web_search`；`流程`/`workflow` 在 `Planner.plan` 固定两步。详见 `lifers/tools.py` 模块注释与 `config/domains.json`。
