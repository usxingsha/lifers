...
# Install extensions listed in rs/.vscode/extensions.json -> recommendations into:
#   - rs/data/extensions (portable, same as run_lifers_cursor.bat)
...
$extJson = Join-Path $rsRoot '.vscode\extensions.json'
...
function Find-CursorExe {
    ...
    @(
        (Join-Path $rsRoot 'shell\Cursor\Cursor.exe'),
...
if (-not $cursor) {
    Write-Warning 'Cursor.exe not found. Install Cursor or place portable build at rs\shell\Cursor\Cursor.exe'
    Write-Host 'You can still install recommendations from the Extensions view when the workspace opens.'
    exit 0
}

$portableExt = Join-Path $rsRoot 'data\extensions'
$portableUd = Join-Path $rsRoot 'data\cursor-user-data'
