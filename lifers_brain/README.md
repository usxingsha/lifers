...
**对话推理分发**：`lifers_brain/taskflow/dialogue_router.py` 的 **`infer_dialogue_route`** 按用户尾句决定 **CHAT_QUICK / WEB_SEARCH / FULL_PIPELINE …**；编排器写入 **`TaskContext.dialogue_route_*`**，并在 stderr 输出 **`LIFERS_PROGRESS dialogue_route`**（Agents Chat「执行过程」可见）。Kali 同步：照旧 **`scripts/lifers_sync_pause_and_brain.ps1`** 或 **`push_brain_and_loop_kali.ps1`**。
