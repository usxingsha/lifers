...
from lifers.transformer_lm import TinyTransformerWeights, build_vocab_from_text
...
def _infer_resume_tier(
    ...
    try:
        ...
        return None
    key = (w.d_model, w.d_ff, w.max_seq)
...
def main() -> int:
    ...
    if not corpus.strip():
        ...
        suite = fallback
        corpus = _load_corpus_from_suite_dir(root, suite)
    # Fallback to training corpus if eval suites are empty
    if not corpus.strip():
        corpus_txt = root / "weights" / "training_corpus.txt"
        if corpus_txt.is_file():
            corpus = corpus_txt.read_text(encoding="utf-8")
            print(f"[lifers-escalate] using training corpus: {corpus_txt} ({len(corpus)} chars)", flush=True)
    if not corpus.strip():
        print("[lifers-escalate] fatal: no corpus available", file=sys.stderr, flush=True)
        return 1

    out = root / "weights" / "lifers_transformer.json"
    out.parent.mkdir(parents=True, exist_ok=True)

    # Crash recovery: check for stale .tmp from power loss / kill
    if recover_stale_tmp(out):
        print("[lifers-escalate] recovered stale .tmp weights from previous crash", flush=True)
    crash = detect_crash(root)
    if crash:
        print(
            f"[lifers-escalate] detected previous crash at ramp {crash.get('ramp_iter','?')}/{crash.get('ramp_max','?')} "
            f"step {crash.get('sgd_step','?')}/{crash.get('sgd_total','?')} loss={crash.get('loss','?')} "
            f"— will resume from saved weights",
            flush=True,
        )
    ...
    for it in range(start_it, max_iters):
        ...
        # Save current tier before training (for OOM detection by outer loop)
        oom_marker = out.parent / ".last_oom_tier"
        if not oom_marker.exists():
            pass  # only written on actual OOM by outer loop
        ...
        try:
            ...
            # Write OOM tier marker so outer loop can auto-cap
            oom_marker = out.parent / ".last_oom_tier"
            try:
                oom_marker.write_text(str(it + 1), encoding="utf-8")
            except OSError:
                pass
            ...
            if pause_mem:
                ...
                print(
                    "[lifers-escalate] wrote control=pause + OOM marker at tier "
                    f"{it + 1} (LIFERS_ESCALATE_PAUSE_ON_MEMERROR default on). "
                    "Outer loop will auto-cap. Tip: export LIFERS_ESCALATE_MAX_TIER to skip this tier.",
                    flush=True,
                )
            break
