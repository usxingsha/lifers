...
在 Lifers 根下的 `scripts` 执行（仓库中多为 `lifers\scripts`）：
...
在 **`lifers\scripts`** 下（需本机 `ssh`/`scp` 可免密登录 Kali）：

```powershell
.\push_brain_and_loop_kali.ps1
```

会做：`package_rs_for_kali.ps1` → `scp` 到 Kali → 解压到 **`~/lifers/lifers`**（**tar 不含 `weights/`，不覆盖 Kali 上已有大权重**）→ 执行 **`remote_kali_bootstrap_train_loop.sh`**：
...
```bash
mkdir -p ~/lifers && tar -xzf ~/lifers_kali.tar.gz -C ~/lifers
bash ~/lifers/lifers/scripts/kali_install_full_train.sh
```

若压缩包不在默认路径，可传入：

```bash
bash ~/lifers/lifers/scripts/kali_install_full_train.sh /path/to/lifers_kali.tar.gz
```
...
```bash
bash ~/lifers/lifers/scripts/kali_install_full_train.sh --enable-boot
```
...
将上述两个文件覆盖到本机仓库的 `lifers\weights\`，与 `config/stack.json` 中 `brain.weights`（`lifers` 产品与 **transformer** 后端共用 `lifers_transformer.json`）一致即可。
...
| 暂停 / 继续 / 停 | `cd ~/lifers/lifers` → `bash scripts/lifers_train_ctl.sh` + 参数 `pause` / `run` / `stop`；多路径同停可用 `bash /tmp/remote_pause_lifers_train.sh`（与 Windows 同步脚本一致） |
